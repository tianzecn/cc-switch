...
- [x] **项目级资源安装功能** - ✅ OpenSpec 已归档: `2026-01-10-add-project-scope-installation`
  - 安装范围选择：全局（global）或项目（project）+ 目标项目路径
  - 项目选择器组件：最近项目列表、手动输入路径、路径有效性验证
  - 范围徽章显示：可视化展示资源安装范围，点击可修改
  - 范围修改对话框：已安装资源可直接切换安装范围
  - 数据库 schema v9→v10 新增 scope/project_path 字段
  - 后端 API：四种资源类型的 change_scope 命令
  - 前端 hooks：useChangeXxxScope mutations
  - i18n 国际化支持（中/英/日）

- [x] **内置仓库管理功能** - ✅ OpenSpec 已归档: `2026-01-10-add-builtin-repos`
  - Skills 和 Commands 支持内置仓库配置
  - 从 JSON 配置文件加载内置仓库列表（Skills 3 个，Commands 5 个）
  - 内置仓库显示「内置」徽章，支持多语言描述
  - 内置仓库可禁用但不可删除
