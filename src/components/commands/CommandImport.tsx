...
/**
 * 未管理 Commands 导入页面
 * 扫描应用目录中未被 SSOT 管理的 Commands，并支持批量导入
 */
export const CommandImport: React.FC<CommandImportProps> = ({ onBack }) => {
  ...
  const filteredCommands = useMemo(() => {
    if (!unmanagedCommands) return [];

    if (!searchQuery) return unmanagedCommands;

    const query = searchQuery.toLowerCase();
    return unmanagedCommands.filter((cmd) => {
  ...
  }, [unmanagedCommands, searchQuery]);

  const handleScan = async () => {
    try {
      await refetch();
      toast.success(t("commands.scanComplete"), { closeButton: true });
    } catch (error) {
  ...
  const handleSelectAll = () => {
    if (selectedIds.size === filteredCommands.length) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(filteredCommands.map((cmd) => cmd.id)));
    }
  };

  const handleImport = async () => {
    if (selectedIds.size === 0) {
      toast.error(t("commands.noCommandsSelected"));
      return;
    }
    ...
    <div className="mx-auto max-w-[72rem] px-6 flex flex-col h-[calc(100vh-8rem)] overflow-hidden">
      ...
      <div className="flex-shrink-0 flex items-center gap-4 py-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-semibold text-foreground">
            {t("commands.importTitle")}
          </h1>
          <p className="text-sm text-muted-foreground">
            {t("commands.importSubtitle")}
          </p>
        </div>
        <Button
          variant="outline"
        ...
        >
          ...
          />
          <span className="ml-2">{t("commands.scan")}</span>
        </Button>
      </div>
        ...
        {filteredCommands.length > 0 && (
          <>
            ...
            >
              ...
              {t("commands.importSelected", { count: selectedIds.size })}
            </Button>
          </>
        )}
      </div>
      ...
      <div className="flex-shrink-0 py-3 glass rounded-xl border border-white/10 mb-4 px-6">
        <div className="text-sm text-muted-foreground">
          {t("commands.foundUnmanaged", { count: filteredCommands.length })} ·{" "}
          {t("commands.selectedCount", { count: selectedIds.size })}
        </div>
      </div>
        ...
        {isLoading || isFetching ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-2">
              {t("commands.clickToScan")}
            </h3>
            <p className="text-muted-foreground text-sm mb-4">
              {t("commands.scanDescription")}
            </p>
            <Button variant="outline" onClick={handleScan}>
              <FolderSearch size={16} className="mr-2" />
              {t("commands.startScan")}
            </Button>
          </div>
        ) : filteredCommands.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-2">
              {t("commands.allManaged")}
            </h3>
            <p className="text-muted-foreground text-sm">
              {t("commands.allManagedDescription")}
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {filteredCommands.map((command) => (
              <UnmanagedCommandCard
                key={command.id}
                command={command}
                isSelected={selectedIds.has(command.id)}
                onToggle={() => handleToggleSelect(command.id)}
              />
            ))}
          </div>
        )}
 ...
 * 未管理命令卡片
 */
interface UnmanagedCommandCardProps {
  command: UnmanagedCommand;
  isSelected: boolean;
  onToggle: () => void;
}

const UnmanagedCommandCard: React.FC<UnmanagedCommandCardProps> = ({
  command,
  isSelected,
  onToggle,
}) => {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-1">
          {command.foundIn.map((app) => (
            <span
              key={app}
              className="px-2 py-0.5 text-xs rounded bg-muted text-muted-foreground"
            >
              {t(`commands.apps.${app.toLowerCase()}`)}
            </span>
          ))}
        </div>
      </div>
